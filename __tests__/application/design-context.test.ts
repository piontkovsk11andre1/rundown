...
describe("design-context canonical workspace resolution", () => {
  it("materializes implementation reconciliation into canonical design/current", () => {
    const fileSystem = new InMemoryFileSystem({
      directories: {
        "/repo/design": [],
        "/repo/implementation": [
        ...
        "/repo/implementation/api": [
          { name: "orders.ts", isDirectory: false, isFile: true },
        ],
      },
      files: {
        "/repo/implementation/README.md": "implementation docs\n",
        "/repo/implementation/api/orders.ts": "export const listOrders = () => [];\n",
      },
      stats: {
        "/repo/design": { isDirectory: true, isFile: false },
        "/repo/implementation": { isDirectory: true, isFile: false },
        "/repo/implementation/api": { isDirectory: true, isFile: false },
      },
    });

    const result = materializeDesignReconciliationFromSource(fileSystem, "/repo", {
      sourceMode: "implementation",
      sourceRoot: "/repo/implementation",
    });
    ...
    expect(normalizePath(result.outputPath)).toBe("/repo/design/current/Implementation.reconciled.md");
    expect(fileSystem.readText("/repo/design/current/Implementation.reconciled.md")).toContain(
      "This file is generated by `rundown migrate --from implementation`.",
    );
    expect(fileSystem.readText("/repo/design/current/Implementation.reconciled.md")).toContain("api/orders.ts");
    expect(fileSystem.readText("/repo/design/current/Implementation.reconciled.md")).toContain("README.md");

    const unchanged = materializeDesignReconciliationFromSource(fileSystem, "/repo", {
      sourceMode: "implementation",
      sourceRoot: "/repo/implementation",
    });
    expect(unchanged.changed).toBe(false);
    expect(normalizePath(unchanged.outputPath)).toBe("/repo/design/current/Implementation.reconciled.md");
  });

  it("materializes prediction reconciliation into configured external design current path", () => {
    const fileSystem = new InMemoryFileSystem({
      directories: {
        "/repo/design": [],
        "/repo/prediction": [
          { name: "checkout", isDirectory: true, isFile: false },
        ],
        "/repo/prediction/checkout": [
          { name: "risk.md", isDirectory: false, isFile: true },
        ],
      },
      files: {
        "/repo/.rundown/config.json": JSON.stringify({
          workspace: {
            design: {
              currentPath: "/external/design-current",
            },
          },
        }),
        "/repo/prediction/checkout/risk.md": "# Checkout Risk\n",
      },
      stats: {
        "/repo/.rundown/config.json": { isDirectory: false, isFile: true },
        "/repo/design": { isDirectory: true, isFile: false },
        "/repo/prediction": { isDirectory: true, isFile: false },
        "/repo/prediction/checkout": { isDirectory: true, isFile: false },
      },
    });

    const result = materializeDesignReconciliationFromSource(fileSystem, "/repo", {
      sourceMode: "prediction",
      sourceRoot: "/repo/prediction",
    });
    ...
    expect(normalizePath(result.outputPath)).toBe("/external/design-current/Prediction.reconciled.md");
    const reconciled = fileSystem.readText("/external/design-current/Prediction.reconciled.md");
    expect(reconciled).toContain("This file is generated by `rundown migrate --from prediction`.");
    expect(reconciled).toContain("checkout/risk.md");
  });

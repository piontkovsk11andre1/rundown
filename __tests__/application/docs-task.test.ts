...
describe("createDocsTask", () => {
  it("publishes docs/current snapshot and keeps no-change no-op behavior", async () => {
    ...
    try {
      ...
      expect(fileSystem.exists("/repo/docs/rev.1")).toBe(true);
      expect(fileSystem.exists("/repo/docs/rev.1/Design.md")).toBe(true);
      expect(fileSystem.readText("/repo/docs/rev.1.meta.json")).toContain("\"label\": \"Baseline\"");
      ...
      expect(fileSystem.exists("/repo/docs/rev.2")).toBe(false);
      expect(outputEvents.some((event) => event.kind === "info" && event.message.includes("No design changes detected in docs/current/"))).toBe(true);
    } finally {
      cwdSpy.mockRestore();
    }
  });

...
async function runCli(args: string[], cwd: string, options?: {
  ...
  const testCliEntrypoint = path.resolve(previousCwd, "dist", "cli.js");
  process.argv = [
    previousArgv[0] ?? "node",
    fs.existsSync(testCliEntrypoint) ? testCliEntrypoint : (previousArgv[1] ?? "/repo/node_modules/.bin/rundown"),
    ...normalizedArgs,
  ];
  process.chdir(cwd);

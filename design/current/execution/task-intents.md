...
| `tool-expansion` | any registered tool prefix that is not `verify`/`memory` (`for:`, `parallel:`, `include:`, `optional:`, `skip:`, …) | tool handler runs; tool decides what executes underneath |
| `fast-execution` | `force:` modifier present | execute → verify; **no** repair retries |
...
2. Modifiers (`profile=`, `force:`) attach metadata but do not change intent.
...
- [ ] force: best-effort cleanup         ← fast-execution

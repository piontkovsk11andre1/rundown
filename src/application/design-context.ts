...
export type DesignReconciliationSourceMode = "implementation" | "prediction";

export interface MaterializeDesignReconciliationResult {
  sourceMode: DesignReconciliationSourceMode;
  sourceRoot: string;
...
export function materializeDesignReconciliationFromSource(
  fileSystem: FileSystem,
  workspaceRoot: string,
  input: {
    sourceMode: DesignReconciliationSourceMode;
...
): MaterializeDesignReconciliationResult {
  const workspace = resolveDesignWorkspaceForRevisions(fileSystem, workspaceRoot, input.invocationRoot);
  const sourceFiles = collectReconciliationSourceFiles(fileSystem, input.sourceRoot);
  const outputFileName = input.sourceMode === "implementation"
    ? "Implementation.reconciled.md"
    : "Prediction.reconciled.md";
  const outputPath = path.join(workspace.currentDir, outputFileName);
  const content = renderDesignReconciliationDocument({
    sourceMode: input.sourceMode,
    sourceRoot: input.sourceRoot,
    sourceFiles,
  });

  fileSystem.mkdir(workspace.currentDir, { recursive: true });
  if (isFile(fileSystem, outputPath) && fileSystem.readText(outputPath) === content) {
    return {
      sourceMode: input.sourceMode,
      sourceRoot: input.sourceRoot,
...
function collectReconciliationSourceFiles(
  fileSystem: FileSystem,
  sourceRoot: string,
): Array<{ relativePath: string; digest: string }> {
  const discovered: Array<{ relativePath: string; digest: string }> = [];
  const queue: string[] = [sourceRoot];

  while (queue.length > 0) {
    const currentDir = queue.shift();
    if (!currentDir || !isDirectory(fileSystem, currentDir)) {
      continue;
    }
    ...
    for (const entry of entries) {
      ...
      discovered.push({
        relativePath,
        digest: digestReconciliationSourceFile(fileSystem, absolutePath),
      });
    }
  }

  return discovered.sort((left, right) => left.relativePath.localeCompare(right.relativePath, undefined, { sensitivity: "base" }));
}

function digestReconciliationSourceFile(fileSystem: FileSystem, filePath: string): string {
  try {
    return createHash("sha1").update(fileSystem.readText(filePath), "utf-8").digest("hex").slice(0, 12);
  } catch {
    const stat = fileSystem.stat(filePath);
    const mtime = typeof stat?.mtimeMs === "number" ? String(stat.mtimeMs) : "unknown";
    return "mtime:" + mtime;
  }
}

function renderDesignReconciliationDocument(input: {
  sourceMode: DesignReconciliationSourceMode;
  sourceRoot: string;
  sourceFiles: Array<{ relativePath: string; digest: string }>;
}): string {
  const title = input.sourceMode === "implementation"
    ? "Implementation Reconciliation"
    : "Prediction Reconciliation";
  const commandSource = input.sourceMode === "implementation"
    ? "implementation"
    : "prediction";
  const sectionTitle = input.sourceMode === "implementation"
    ? "Implementation Snapshot"
    : "Prediction Snapshot";
  const lines = input.sourceFiles.length > 0
    ? input.sourceFiles.map((entry) => "- " + entry.relativePath + " (" + entry.digest + ")")
    : ["- (no files found)"];

  return [
    "# " + title,
    "",
    "This file is generated by `rundown migrate --from " + commandSource + "`.",
    "",
    "- Source root: `" + input.sourceRoot.replace(/\\/g, "/") + "`",
    "- File count: `" + String(input.sourceFiles.length) + "`",
    "",
    "## " + sectionTitle,
    "",
    ...lines,

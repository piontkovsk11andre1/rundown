...
export async function dispatchTaskExecution(params: {
  ...
  if (task.isInlineCli) {
    ...
    const delegatedRundownCommand = parseDelegatedRundownCommand(inlineCliCommand);
    const cliResult = delegatedRundownCommand
      ? await dependencies.workerExecutor.executeRundownTask(
        delegatedRundownCommand.subcommand,
        delegatedRundownCommand.args,
        inlineCliCwd,
        {
          env: executionEnv,
          artifactContext,
          keepArtifacts,
          artifactExtra: { taskType: "inline-cli", delegatedRundown: true },
      ...
      : await dependencies.workerExecutor.executeInlineCli(inlineCliCommand, inlineCliCwd, {
        env: executionEnv,
        artifactContext,
        keepArtifacts,
...
function parseDelegatedRundownCommand(command: string): { subcommand: DelegatedRundownSubcommand; args: string[] } | null {
  const tokens = tokenizeShellLikeCommand(command);
  if (tokens.length < 2) {
    return null;
  }

  if (!/(?:^|[\\/])rundown(?:\.(?:cmd|ps1|bat|exe))?$/i.test(tokens[0] ?? "")) {
    return null;
  }

  const subcommand = (tokens[1] ?? "").toLowerCase();
  if (subcommand !== "run" && subcommand !== "make" && subcommand !== "do" && subcommand !== "research") {
    return null;
  }

  return { subcommand, args: tokens.slice(2) };
}

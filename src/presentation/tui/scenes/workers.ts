...
export function renderWorkersSceneLines({ state, sectionGap = 1 } = {}) {
  ...
  const hasInteractive = Object.hasOwn(workers, "interactive");
  const fallbackConfig = config.fallbacks && typeof config.fallbacks === "object" && !Array.isArray(config.fallbacks)
    ? config.fallbacks
    : {};
  ...
  if (!hasDefault && !hasInteractive && fallbacks.length === 0 && !hasWorkerTimeout) {
    ...
    if (hasInteractive) {
      const value = formatCommand(workers.interactive);
      pushLabeledRow(lines, "interactive", value, formatHealth(healthByIdentity.get(value)));
    }
